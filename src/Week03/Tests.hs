...
module WeekO3.Tests where

import Control.Monad (replicateM, unless)
import Plutus.Model
import Plutus.Moderl.Fork.Ledger.Slot (Slot)
...
setupUsers :: PubKeyHash -> Run [ PubKeyHash ]
...
homework1 cfg = do
    ...
    [ testGroup "Beneficiary 1 signing"
        ...
        , bad "Deadline: 6000; TxValidRange (5000, 7000)" $ testBeneficiary1 6000 (-999) 1001 1
    ...
    , testGroup "Beneficiary 2 signing"
        ...
        , bad "Deadline: 6000; TxValidRange (5000, 5999)" $ testBeneficiary1 6000 (-999) 0 0
        , bad "Deadline: 5000; TxValidRange (5000, 6000)" $ testBeneficiary1 5000 (-999) 1 0
        , bad "Deadline: 5000; TxValidRange (5001, 6000)" $ testBeneficiary1 5000 (-998) 1 0
        , bad "Deadline: 5000; TxValidRange (5999, 6000)" $ testBeneficiary1 5000 0 1 0
        ]
...
testBeneficiary1 deadline curMinT curMaxT wSlot = do
    users <- setupUsers
    let [ u1, u2, u3 ] = users
    let dat = H1.VestingDatum u1 u2 deadline
...
testHomework1 sigUser receiver dat curMinT curMaxT wSlot = do
    ...
    checkBalance (gives script1 (txOutValue vestOut) receiver) $ do
        ...
        tx <- validateIn range $ 
            claimingTx1 receiver dat vestRef (txOutValue vestOut)
            submitTx sigUser tx
...
testHomework2 dat curMinT curMaxT wSlot = do
    ...
    checkBalance (gives (script2 u2) (txOutValue vestOut) u2) $ do
        range <- currentTimeInterval curMinT curMaxT
        tx <- validateIn range $ 
            claimingTx2 u2 dat vestRef (txOutValue vestOut)
            submitTx u2 tx
